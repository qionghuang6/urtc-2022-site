...
export default function Calendar({events}) {
    ...
    return (
        <>
            {events?.items.filter(event => new Date(event.end.dateTime).getTime() > Date.now()).map(event => (
                <GlassBox key={event.id} sx={{ display: 'inline-block', margin: '8px', paddingX: '20px', paddingY: '12px', width: mobile ? '95%' : 'fit-content'}} background='linear-gradient(135deg, rgba(33, 147, 200, 0.6), rgb(109, 213, 237, 0.6))'>
                    ...
                    <Typography align="left" variant="body1" color="white">{new Date(event.start.dateTime).toLocaleDateString()}</Typography>
